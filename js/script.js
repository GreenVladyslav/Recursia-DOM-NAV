...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addForm.addEventListener('submit', (event) => {
        ...
        const favotie = checkbox.checked;

        if (newFilm !== '') {
            
            if (newFilm.length > 11) {
                // newFilm = `${newFilm.substring(0, 11)}...`;
                newFilm = newFilm.slice(0, 11) + '...';
            }
            ...
            createMoveList(movieList, movieDB.movies);
        }
    ...
    const makeChanges = (genre) => {
        // genre.textContent = 'Драма';
        // genre.innerHTML = 'Драма';
    
        genre.textContent = '';
        genre.insertAdjacentText('afterbegin', 'Драма');
    };

    function replaceBackground(bg) {
    ...
    replaceBackground(bg);
    
    const sortArr = (arr) => {
        arr.sort();
    };

    function createMoveList(parent, films) {
        parent.innerHTML = "";
        ...
        document.querySelectorAll('.delete').forEach((btn, index) => {
            btn.addEventListener('click', () => {
                ...
                createMoveList(parent, films); /* Рекурсия */
            });
        });
    }
    deletePromo(promo);
